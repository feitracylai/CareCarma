...
class DetailView extends Widget
{
     ...
     */
    public $template = '<tr><th style="font-size: 15px;"><strong>{label}</strong></th><td >{value}</td></tr>';
    /**
     ...
     * @return string the rendering result. If {value} is '', don't display.
     */
    protected function renderAttribute($attribute, $index)
    {
        if (is_string($this->template)) {
            if ($attribute['value'] == '') {
                return;
            } else {return strtr($this->template, [
